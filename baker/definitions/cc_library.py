...
class CCLibrary(Module):
    ...
    def convert_to_cmake(self) -> list[str]:
        ...
        if not self._module.name.endswith("_static"):
            ...
            #lines.append(f'target_link_libraries({name}-shared INTERFACE $<TARGET_PROPERTY:{object},INTERFACE_LINK_LIBRARIES>)')
            #lines.append(f'target_include_directories({name}-shared INTERFACE $<TARGET_PROPERTY:{object},INTERFACE_INCLUDE_DIRECTORIES>)')
        ...
        if not self._module.name.endswith("_shared"):
            ...
            #lines.append(f'target_link_libraries({name}-static INTERFACE $<TARGET_PROPERTY:{object},INTERFACE_LINK_LIBRARIES>)')
            #lines.append(f'target_include_directories({name}-static INTERFACE $<TARGET_PROPERTY:{object},INTERFACE_INCLUDE_DIRECTORIES>)')
