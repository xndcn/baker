...
function(baker_inherit_defaults target)
    ...
    get_property(all_keys TARGET ${target} PROPERTY _ALL_KEYS_)
    foreach(defaults IN LISTS defaults_list)
        get_property(keys TARGET ${defaults} PROPERTY _ALL_KEYS_)
        foreach(key IN LISTS keys)
            get_property(value TARGET ${defaults} PROPERTY ${key})
            set_property(TARGET ${target} APPEND PROPERTY ${key} ${value})
        endforeach()
        list(APPEND all_keys ${keys})
    endforeach()
    list(REMOVE_DUPLICATES all_keys)
    set_property(TARGET ${target} PROPERTY _ALL_KEYS_ ${all_keys})
endfunction()
