...
function(baker_inherit_defaults target)
    ...
    get_property(all_single_keys TARGET ${target} PROPERTY _ALL_SINGLE_KEYS_)
    get_property(all_list_keys TARGET ${target} PROPERTY _ALL_LIST_KEYS_)
    foreach(defaults IN LISTS defaults_list)
        get_property(keys TARGET ${defaults} PROPERTY _ALL_SINGLE_KEYS_)
        foreach(key IN LISTS keys)
            get_property(value TARGET ${defaults} PROPERTY ${key})
            set_property(TARGET ${target} PROPERTY ${key} ${value})
        endforeach()
        list(APPEND all_single_keys ${keys})

        get_property(keys TARGET ${defaults} PROPERTY _ALL_LIST_KEYS_)
        foreach(key IN LISTS keys)
            get_property(value TARGET ${defaults} PROPERTY ${key})
            set_property(TARGET ${target} APPEND PROPERTY ${key} ${value})
        endforeach()
        list(APPEND all_list_keys ${keys})
    endforeach()
    list(REMOVE_DUPLICATES all_single_keys)
    set_property(TARGET ${target} PROPERTY _ALL_SINGLE_KEYS_ ${all_single_keys})
    list(REMOVE_DUPLICATES all_list_keys)
    set_property(TARGET ${target} PROPERTY _ALL_LIST_KEYS_ ${all_list_keys})
endfunction()
...
function(baker_defaults)
    ...
    foreach(key IN LISTS ARG__ALL_SINGLE_KEYS_)
        set_property(TARGET ${ARG_name} PROPERTY _${key} ${ARG_${key}})
    endforeach()
    foreach(key IN LISTS ARG__ALL_LIST_KEYS_)
        set_property(TARGET ${ARG_name} APPEND PROPERTY _${key} ${ARG_${key}})
    endforeach()
    list(TRANSFORM ARG__ALL_SINGLE_KEYS_ PREPEND _)
    set_property(TARGET ${ARG_name} PROPERTY _ALL_SINGLE_KEYS_ ${ARG__ALL_SINGLE_KEYS_})
    list(TRANSFORM ARG__ALL_LIST_KEYS_ PREPEND _)
    set_property(TARGET ${ARG_name} PROPERTY _ALL_LIST_KEYS_ ${ARG__ALL_LIST_KEYS_})

    target_sources(${ARG_name} INTERFACE ${ARG_srcs})
endfunction()